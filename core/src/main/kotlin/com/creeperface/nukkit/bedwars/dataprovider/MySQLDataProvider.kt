...
internal class MySQLDataProvider(private val configuration: Configuration) : DataProvider {
    ...
    override fun init() {
        ...
        connection.use { con ->
            ...
            con.prepareStatement(
                "CREATE TABLE IF NOT EXISTS $ARENAS_TABLE (" +
                        "id INT PRIMARY KEY auto_increment," +
                        "name VARCHAR(64) NOT NULL UNIQUE," +
                        "data TEXT NOT NULL," +
                        "last_update TIMESTAMP," +
    ...
    override suspend fun getData(identifier: String): Stats? {
        connection.use { con ->
            con.prepareStatement("SELECT (kills, deaths, wins, losses, beds, placed, broken, games) FROM $STATS_TABLE WHERE identifier = ?")
                .use { statement ->
                    statement.setString(1, identifier)
    ...
    override suspend fun getDataByName(name: String): Stats? {
        connection.use { con ->
            con.prepareStatement("SELECT (kills, deaths, wins, losses, beds, placed, broken, games) FROM $STATS_TABLE WHERE name = ?")
                .use { statement ->
